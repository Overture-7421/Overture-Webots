...
NTMotor::NTMotor(Robot* robot, const NTMotor::Config& config) {
    ...
    if (config.Model == "Kraken") {
        ...
        std::cerr << "NTMotorPlugin used invalid FRC motor model " << config.Model << "!!!!";
        return;
    }
...
void NTMotor::Update() {
    ...
    auto current = motorModel.Current(units::radians_per_second_t(jointTurnsPerS * 2.0 * M_PI), appliedVoltage);
...
void to_json(nlohmann::json& j, const NTMotor::Config& c) {
    j = nlohmann::json{
        ...
        {"suffix", c.NtSuffix},
        {"model", c.Model},
        {"gear_ratio", c.GearRatio},
        {"count", c.MotorCount},
        {"mechanically_inverted", c.MechanicallyInverted}
    };
};
